...
1. **Allow editing or deleting at any index for duplicate articles**: Currently, the user can only edit or delete the first article when there are multiple articles with the same headline. It can be improved by allowing the user to edit or delete any article with the same headline.
1. **Prohibit using duplicate prefixes for the same attribute**: Currently, the user can use duplicate prefixes for the same attribute when adding or editing persons or articles. It can be improved by prohibiting the user from using duplicate prefixes for the same attribute.
1. **Allow delete multiple people and articles within single command**: Currently, the user can only delete one person or article at a time. It can be improved by allowing the user to delete multiple people or articles within a single command.
